...
which is possible to transmit information &ndash; we distinguish between
the speed of transmission and modulation:

$} &ndash; express the number of changes *a* of
...
$}
&ndash; indicates the amount of information
...
radio on VHF has bit rate 1200&thinsp;bps and the used modulation is AFSK (Audio
Frequency-Shift Keying). Frequencies carrying information are two &ndash;
2200&thinsp;Hz for mark (log. 1) and 1200&thinsp;Hz for space (log. 0). We know
$v_p = 1200$&thinsp;bps, $m=2$, so symbol rate is equal to bit rate:

```

1&thinsp;200&thinsp;\hbox{Bd}.

```

A packet radio is based on ITU-T V.23 specification for telephone
modems, where bandwidth is limited to about 4&thinsp;kHz. Modern dial-up
modems, but have a much higher bit rates, up to 56&thinsp;kbps and the
bandwidth remains 4&thinsp;kHz. How is that possible?
...
rate up to 9600&thinsp;bps. There is used *QAM (Quadrature Amplitude Modulation)*, which in case of QAM-16 has 16 states per one modulation symbol.
The symbol rate in this case is:

```

2&thinsp;400&thinsp;\hbox{Bd}.
