# Facsimile &ndash; Radiofax

Facsimile (from latin *facere* &ndash; make and *simile* &ndash; similar) is one
...
meteorological maps and informations, hence the name *Weather Facsimile (WEFAX)* follows from it. The *radiofax* can be used by radio amateurs
too.

## The history of image transmission
...
Figure 1: Bakewell's image telegraph.

[Image: fax/obr/bakewell]
...
device named *panthèlègraph*. With pantèlègraph could be sent
images or texts.

Caselli received enthusiastic support from the French emperor Napoleon&thinsp;III.
...
Figure 2: The principle of an electromechanical image capture.

[Image: fax/obr/fax.pdf]
...
on CCITT recommendations { Facsimile CCITT Group 1 (T.2)} of 1968, the short
wave recommendation is described . The CCITT recommends
the frequency 1500&thinsp;Hz for white and 2300&thinsp;Hz for black. In North America it is
1500&thinsp;Hz for white and 2300&thinsp;Hz or 2400&thinsp;Hz for black. The transmission speed
is 180&thinsp;lpm. The fax machines of this type could be adjusted for amateur operation.
The later recommendations T.3, T.4 or T.30 can not be used on HF and there are
used in telephone lines.

The frequency modulation *F3C* is used for shortwave transmission (*F* &ndash; frequency modulation,
$3$ &ndash; single channel containing analog information, *C* &ndash; facsimile). The transmitter modulates
the frequency of carrier in the range $$400&thinsp;Hz on shortwaves and $$150&thinsp;Hz on
long waves. This range is called *signal deviation*.

The fax signal can be created by direct modulation of broadcast frequency or by
frequency modulation of subcarrier 1900&thinsp;Hz. Then the transmitter changes
frequency between black and white color. Black color corresponds to 1500&thinsp;Hz
and white 2300&thinsp;Hz.
...
determines the brightness. For black color it is 4&thinsp;% level and for white it
is 90&thinsp;% to 100&thinsp;%. The negative modulation inverts levels, the minimal value
is for white and the maximal value for black.

Most commercial stations use *APT &ndash; Automatic Picture Transmission* for a
fully automated reception without requiring the presence of an receiver
operator. The image transmission begins with *start tone*, when transmitter
modulates the carrier with some frequency, mostly 300&thinsp;Hz (changing the maximal
levels of modulation 300$$ in a second). This signal is recognized be the
receiving device and it switches from stand-by mode to working mode and waits for
*phasing signal*.

Figure 3: The start of facsimile transmission.

[Image: fax/obr/fax_apt.pdf]

The phasing signal is used for synchronization and it is broadcast few second
before an image. Normally consist of rows of 95&thinsp;% black and 5&thinsp;% white. It
...
modulation 450&thinsp;Hz and switches the receiving device back to stand-by mode.
...
about 1900&thinsp;Hz below the station frequency. So if the station uses frequency
3855.0&thinsp;kHz, you must tune in USB on 3853.1&thinsp;kHz (i.e. 1900 Hz below),
...
Figure 4: Relation between reception and transmission frequencies.

[Image: fax/obr/usb_fax_channel.pdf]
...
Figure 5: Modern receiver Sony CRF-V21 for WEFAX and RTTY reception equipped with printer.

[Image: fax/obr/crfv21cs.jpg]

The most important parameters of the transmission are the speed and *index of cooperation &ndash; IOC*. The IOC relates with a horizontal scan rate and
...
Figure 6: Typical synoptic map transmitted on HF by facsimile, here from station DDH3, line speed 120&thinsp;lpm and IOC 576.

[Image: fax/obr/offenbach_synopt.png]
...
stations use most often 120&thinsp;lpm, in eastern European countries and in
post-soviet states it is 90&thinsp;lpm, news agencies use 60&thinsp;lpm and meteo satellites
240&thinsp;lpm.
...
#### Facsimile transmission modes

{ Name IOC lpm APT start d APT stop d Note Wefax 288 288 120&thinsp;/&thinsp;90&thinsp;/&thinsp;60 675 Hz 3 s 450 Hz 3 s Wefax 576 576 120&thinsp;/&thinsp;90&thinsp;/&thinsp;60 300 Hz 3 s 450 Hz 3 s Ham Color 204 360 200 Hz 5 s 450 Hz 5 s color Ham 288b 240 240&thinsp;/&thinsp;120 675 Hz 5 s 450 Hz 5 s Color 240 288 240 200 Hz 3 s 450 Hz 5 s color FAX 480 204 480 500 Hz 3 s 450 Hz 3 s Photopress 352 60 ? ? 450 Hz ? *d* &ndash; minimal duration of APT signal }
...
Figure 7: The station schedule of GYA.

[Image: fax/obr/northwood_sched.png]
...
frequencies 3855.0, 7880,0 a 13&thinsp;882,5 kHz. As already announced, station
always receive in USB mode and tune it 1900&thinsp;Hz below. Therefore DDH3 tune on
3853.1&thinsp;kHz. Its speed is 120&thinsp;lpm and IOC 576.

Figure 8: Test chart of DDH3 transmitted daily in 11:10 UTC.

[Image: fax/obr/test_chart_offenbach_c.png]

Another station is an Englich GYA broadcasting from Northwood (120/576) on
frequencies 2618.5, 4610.0, 8040.0, 11085.5&thinsp;kHz (active are at least two
channels simultaneously).

From long distance stations cab be received almost daily Tokyo station JMH4.
Of the three transmitters is the best JMH4 on 13597.0&thinsp;kHz with 5&thinsp;kW output
power. Besides the usual synoptic chart there are available satellite imagery
too.

Figure 9: Test chart from JMH4 available daily at 13:00 UTC.

[Image: fax/obr/tokyo_test_chart.png]

Next Tokyo station is JJC, it is Kyodo News Agency. Transmission speed is
60&thinsp;lpm and IOC 576, sometimes 120&thinsp;lpm when weather charts are posted. The
station broadcast simultaneously on several frequencies: 12745.5, 16971.0,
17069.6, 22542.0&thinsp;kHz, by listening find the active frequency. Interestingly,
...
Figure 10: Typical JJC transmission.

[Image: fax/obr/kyodo_news_jap.png]

For those who deal with DX radio reception on the HF While listening to the
interesting DXů will be rewarded by received image. Complete list of stations sorted by country or frequency see .

The detail list of frequencies and station schedule can be find in publication
*Marine Worldwide Radiofacsimile Broadcast Schedules*, which is published
...
of meteorological data is the website of the *World Meteorological Organizations W.M.O.* Besides general information about meteorology there are
...
[](11.4_satellite-imagery-retransmission.md ":include")

[](11.5_hamradio-facsimile-operations.md ":include")

[](11.6_international-fax-standard.md ":include")
