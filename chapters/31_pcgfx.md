...
320$$240 pixels. You can achieve this in almost all SSTV programs, but
...
procedure – *discrete convolution*. A way the image is processed
...
e.g. matrix $_2$ in use pixel and
...
The next useful matrices for averaging are $_2$ and $_3$,
...
are two convolution kernels for sharpening $_4$ and $_5$
...
when the filter $_5$ is used, so there appears teeth on vertical
