...
black and white transmission and helped to develop color image
transmission.
...
companies &ndash; the American *Robot Research Inc.* and the
German *Wraase Electronic* led by radio amateur Volker
Wraase, DL2RZ. Each of them introduced an SSTV converter that
used each company's transmission system. The systems are
different in the usage of color coding, scan line formats, and
synchronization methods. Their converters provide several modes.
*Mode* denotes a format of image transmission, its
...
transmission time, resolution, color coding, etc. The vast
majority of them are unique and incompatible&hellip;

You might be a little scared by the previous paragraphs, but let
me reassure you that only a few modes are actually used.

European amateurs widely used the SSTV mode called *Martin M1*, but in recent times other modes; *Martin M2* and *Scottie S2* are also in use. A special mode used *Scottie DX*; is characterized by the very high image quality. And the
mode *Robot 36 Color* is undertaken in space communication.

Fortunately, all modern converters and computer software can
operate with these popular modes, so the problem that two
stations can not establish the QSO should not occur.
...
*attenuation*. Attenuation defines how much the
communication channel reduces a transferred signal. Another
important characteristic is the *bias*, which refers to the
...
used for modulation, in our case 1500&thinsp;Hz to 2300&thinsp;Hz, but
also on the signal spectrum.

*Fourier analysis* is used to determine the spectrum
bandwidth. The analysis can express any waveform in the form of
the sum of a large number of sine waves &ndash; harmonic components.
...
in [**Sampling**](chapters/7_computer-operations#sampling)).

*Bandwidth* can be seen as a characteristic of the
transmission path given by the range of the signal spectrum.

The basic rule for the required bandwidth is called *Nyquist rate*. Its definition is that optimal bandwidth equals half of
the modulation speed. The necessary bandwidth indeed increases
with the amount of transferred information per time unit.

### Modulation techniques of analog SSTV
...
Frequencies above 2500&thinsp;Hz are strongly suppressed, so the
frequency of white color, the maximal level of an SSTV signal,
was chosen at 2300&thinsp;Hz.
...
1900&thinsp;Hz &ndash; *sub-carrier*. This modulation method is called
*Sub-carrier frequency modulation (SFCM)*.
...
the range of 1.0 to 3.2&thinsp;kHz and depends on the SSTV mode,
transmission speed and also on image content, see [**fig. 3.1**](#figure_sstv_spectrum).

<div id="figure_sstv_spectrum" class="figure-combo">

  <img src="res/sstv/sstv_spectrum.svg"/>

  <div><b>Figure 3.1:</b> The SSTV frequency spectre for two various images transferred in Martin M1 mode.</div>
...
- *J* &ndash; Carrier modulation: Single-sideband with a suppressed carrier.
- *3* &ndash; Nature of modulating signal: One channel containing analog information.
- *F* &ndash; Detail of signal: television signals.

In the case of an SSTV transmission via a frequency modulated
(FM) channel, the emission is classified as *F3F* and *A3F* for amplitude modulation (AM) with both sidebands.

## Image resolution

*Resolution* is a feature that tells what amount of details is
possible to store in an image, see [**fig. 3.2**](#figure_sstv_ara). The
resolution has two parameters: *horizontal resolution* &ndash; the number
of image columns × number of image lines &ndash; *the vertical resolution*.

<div id="figure_sstv_ara" class="figure-combo">

  <div class="figure-item">
    <div class="figure-wrapper">
      <img src="res/sstv/ara50x38.png" />
    </div>
    <div>50&thinsp;×&thinsp;38</div>
  </div>

  <div class="figure-item">
    <div class="figure-wrapper">
      <img src="res/sstv/ara120x90.png" />
    </div>
    <div>120&thinsp;×&thinsp;90</div>
  </div>

  <div><b>Figure 3.2:</b> Image quality depends on resolution.</div>
...
The SSTV is an *analog* mode and cannot transfer images
...
displayed in a 4&thinsp;:&thinsp;3 aspect ratio on a screen. We can then say
that the number of columns is 240&thinsp;×&thinsp;4/3&thinsp;=&thinsp;320. This value
then corresponds to a theoretical resolution, but not a *real* image resolution.

The test chart ([**fig. 3.3**](#figure_sstv_resol)) is used to qualify
...
image with normal photography in [**fig. 3.1**](#figure_sstv_spectrum).

<div id="figure_sstv_resol" class="figure-combo" style="row-gap: 1rem; align-items: flex-end;">

  <style>
    #figure_sstv_resol>img {
      width: 50%;
    }

    #figure_sstv_resol>img+div {
      width: 12rem;
      max-width: 50%;
      text-align: initial;
    }
  ...
  <img src="res/sstv/resol/original.png"/>
  <div>Test pattern original</div>

  <img src="res/sstv/resol/m2.png"/>
  <div>Martin M2(∼220&thinsp;μs)</div>

  <img src="res/sstv/resol/r36.png"/>
  <div>Robot 36 Color(∼280&thinsp;μs)</div>

  <img src="res/sstv/resol/m1.png"/>
  <div>Martin M1(∼450&thinsp;μs)</div>

  <img src="res/sstv/resol/mp115.png"/>
  <div>MP115(∼680&thinsp;μs)</div>

  <img src="res/sstv/resol/dx.png"/>
  <div>Scottie DX(∼980&thinsp;μs)</div>

  <div><b>Figure 3.3:</b> Horizontal resolution comparison for several SSTV modes.</div>

</div>

All SSTV modes in [**fig. 3.3**](#figure_sstv_resol) have 320 columns.
...
picture in [**fig. 3.4**](#figure_sstv_rozliseni). The last two modes listed have
...
<div id="figure_sstv_rozliseni" class="figure-combo">

  <div class="figure-item">
    <div class="figure-wrapper">
      <img src="res/sstv/rozliseni_m1.png" />
    </div>
    <div>Martin M1</div>
  </div>
  ...
  <div class="figure-item">
    <div class="figure-wrapper">
      <img src="res/sstv/rozliseni_m2.png" />
    </div>
    <div>Martin M2</div>
  </div>

  <div><b>Figure 3.4:</b> The comparison of two modes in real
conditions of 14&thinsp;MHz band.</div>

</div>

[](3.3_line-speed.md ":include")