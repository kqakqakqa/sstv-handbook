...
in number of modulation states &ndash; QAM-4, QAM-16 and QAM-64.
...
the word length $l = <sub>2</sub> m = <sub>2</sub> 4 = 2$, for QAM-16 is $l=4$
...
is . The information words with 4-bit length
are divided on two parts in mapping circuit and first 2-bit combination
is coded in *pulse amplitude modulation (PAM)* into one of four
levels. The way how to code input bit quaternion $\{i<sub>0</sub>, i<sub>1</sub>, q<sub>0</sub>, q<sub>1</sub>}$ is defined by *constellation diagram*,
=00$ and this corresponds to $I=3, Q=3$, the next value 13, in binary
1101 corresponds $i<sub>0</sub>i<sub>1</sub> = 11$ output $I=-3$ and for $q<sub>0</sub>q<sub>1</sub> =01$
output $Q=-1$, etc.
...
Figure 2: The constellation diagram for QAM-16 with bit order $\{i<sub>0</sub>, i<sub>1</sub>, q<sub>0</sub>, q<sub>1</sub>\
...
### Orthogonal frequency-division multiplexing &ndash; OFDM
