...
The image with resolution 320&thinsp;×&thinsp;240 with a color depth of 16 million
colors ($256^3$) takes 230400&thinsp;Bytes (320&thinsp;×&thinsp;240&thinsp;×&thinsp;3) without
...
The graph of negative logarithm see on &ndash;
...
$a<sub>1</sub>, a<sub>2</sub>, a<sub>8</sub>$ and probability of their occurrence is same:
...
3-bit length: 000, 001, 010, Maximum entropy is reached when the
...
example we need to transfer messages $a<sub>1</sub>, a<sub>2</sub>, a<sub>7</sub>$. Their
probabilities are
$p(a<sub>1</sub>)=0.235$,
$p(a<sub>2</sub>)=0.206$,
$p(a<sub>3</sub>)=0.176$,
...
\eqalign{ H & = -\displaystyle\sum_{i=1}^7 p(a_i)\log<sub>2</sub>{p(a_i)} = \cr ~ & = - \big(0.235\cdot{}(-2.09)+ 0.206\cdot{}(-2.28)+ 0.176\cdot{}(-2.50)+ 0.147\cdot{}(-2.76)+ \cr & 0.118\cdot{}(-3.08)+ 0.059\cdot{}(-4.08)+ 0.029\cdot{}(-5.08)+ 0.029\cdot{}(-5.08) \big)&thinsp;\mathrm{bits} \cr H & \approx -2.712&thinsp;\mathrm{bits} \cr }
...
be expressed with code words of 3-bit length. Its whole length is $3 34 =
102$&thinsp;bits.
...
A (.&ndash;), I (..) have assigned shorter codes and the less frequent, such as H
(....), J (.&ndash;&thinsp;&ndash;&thinsp;&ndash;), F (..&ndash;.) longer codes.
...
4&thinsp;:&thinsp;2&thinsp;:&thinsp;0 reduces size of file, but itself is not enough. The image
