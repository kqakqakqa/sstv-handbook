...
needs to support 1024$$768 resolution in
...
Figure 1: The conversion of an analog signal into numeric data.

[Image: software/obr/ac_prevod.pdf]
...
example below. The signal in a expresses the
dependency between time $t$ and amplitude $A$. Using Fourier's analysis, we can find
(see ), that the signal contains two harmonic components, showed
in b.

Figure 2: Example of signal.

[Image: software/obr/signal-A.pdf]

[Image: software/obr/signal-B.pdf]

By using Fourier's transformation, the signal can also be expressed as a
dependency between amplitude $A$ and frequency $f$ – *signal spectrum*.
Both frequency components are evident $f_1$ and $f_2=f_{max}$ in the
signal in .

Figure 3: The frequency spectrum of signal in {fig.

[Image: software/obr/spektrum.pdf]

For explicit signal reconstruction the condition $f_{s} > 2f_{max}$ must be
satisfied, see fig. . If a sampling frequency is
lower then $2f_{max}$ then the higher frequency components are lost. This
error is called *aliasing*.

Figure 4: The signal sampled with a frequency higher than $2f_{max

[Image: software/obr/signal-VZ.pdf]

Figure 5: The sample rate does not meet the Shannon's theorem.

[Image: software/obr/signal-VZbad.pdf]
...
$5.0/(2^8-1) 0.02$\,V. The 8bit converter cannon distinguish
voltage levels lower then 0.02\,V. So for input voltage 3.111\,V it could
find corresponding numeric value $10011110_2\, 3.098$\,V
or $10011111_2\, 3.118$\,V because less significant bit is
...
10\,k$$ which gives max. output voltage 100\,mV. This output could be also
...
Figure 6: Basic interface between transceiver and sound card.

[Image: software/obr/sound_card.pdf]

For the transmission it is possible to use *Line Out* with impedance
about 600\,$$. The *Line Out* can be connected to microphone
...
Figure 7: Software volume control.

[Image: software/obr/hlas.png]
...
TRX instruction or you can connect trimming resistor about 15\,k$$ and
...
Figure 8: The galvanic separation of transceiver and sound card.

[Image: software/obr/sound_if.pdf]
...
in . If a transmitting station has higher timing (and reception
lower) the image slants to the right (a) in opposite situation to the left
(b).

Figure 9: Image slant distortion when inaccurate timing is used for free-run modes.

[Image: software/obr/slant1.png]

[Image: software/obr/slant2.png]
...
Figure 10: The configuration of accurate timing with { RMW

[Image: software/obr/rmw_nastaveni.png]
...
$_f=-0{,}55$\,Hz. Some program this measure in *parts per million (ppm)* unit. The ppm deviance can be computed:
...
see fig. . The spectroscope shows frequency band from 1000\,Hz
...
Figure 11: Spectroscopes in common SSTV programs.

[Image: software/obr/chromapix.png]

[Image: software/obr/mmsstv.png]

[Image: software/obr/jvfax1.png]

[Image: software/obr/mscan.png]

[Image: software/obr/qsstv.png]

[Image: software/obr/mixw.png]
