...
sound card and download some SSTV related software. Then tune into 14.230&thinsp;MHz
...
- to configure an accurate sampling frequency for reception and transmission,
- to set proper sound card volume levels,
- to use the tuning indicator,
- to manually change the transmission mode,
- to load and save images in common graphic formats,
- to create an image gallery for transmission and
- to add text into transmitted images.
...
PCs with old 386 processors. The *Hamcomm* modem and some other
popular programs from the nineties run on DOS. In this case,
4&thinsp;MB of RAM, a hundred megabytes hard disk and SVGA
...
should be configured with 150&thinsp;MHz Pentium, 64&thinsp;MB
RAM and few gigabytes hard disk. A graphics card
needs to support 1024&thinsp;×&thinsp;768 resolution in
64 thousands or 16.7 million color mode. The minimal operating
system is Windows&thinsp;95 OSR&thinsp;2, but some new programs may not
run in Win95. I recommend having better hardware but you don't
need the latest mega-hyper model.

There are also software products for Mac&thinsp;OS&thinsp;X and for GNU/Linux, but
the largest selection is for Microsoft Windows.

## Sound card as a modem
...
in hamshacks is as a MODEM. The modem &ndash; MOdulator/DEModulator allows computer
...
The digital conversion process begins with *sampling*. Sampling
is an activity which periodically scans the current value of the analog
signal. For example, this happens 11025 times per second or
depends on a user defined sample rate supported by the sound card.
The sampling frequency of sound cards ranges from
8&thinsp;kHz suitable for internet telephony up to 96&thinsp;kHz designated
for more exacting requirements of recording studios.

<div class="figure-combo">

  <img src="res/software/ac_prevod.svg"/>

  <div><b>Figure 7.1:</b> The conversion of an analog signal into numeric data.</div>
...
highest frequency *f*<sub>max</sub>, can be clearly reconstructed from a sequence
of evenly spaced samples with a sampling frequency *f*<sub>s</sub> greater than double
*f*<sub>max</sub>:

<div class="math-block">
  <i>f</i><sub>s</sub>&thinsp;&gt;&thinsp;2<i>f</i><sub>max</sub>
</div>

You can see the importance of Shannon's theorem in the
example below. The signal in [**fig. 7.2**a](#figure_sampled_a) expresses the
dependency between time *t* and amplitude *A*. Using Fourier's analysis, we can find
(see [**Bandwidth**](chapters/3_modern-sstv-features#bandwidth)), that the signal contains two harmonic components, showed
in [**fig. 7.2**b](#figure_sampled_b).

<div class="figure-combo">

  <div id="figure_sampled_a" class="figure-item">
    <div class="figure-wrapper">
      <img src="res/software/signal-A.svg" />
    </div>
    <div>a) Surveyed signal</div>
  </div>

  <div id="figure_sampled_b" class="figure-item">
    <div class="figure-wrapper">
      <img src="res/software/signal-B.svg" />
    </div>
    <div>b) Spectral components</div>
  </div>

  <div><b>Figure 7.2:</b> Example of signal.</div>

</div>

By using Fourier's transformation, the signal can also be expressed as a
dependency between amplitude *A* and frequency *f* &ndash; *signal spectrum*.
Both frequency components are evident *f*<sub>1</sub> and *f*<sub>2</sub>=*f*<sub>max</sub> in the
signal in [**fig. 7.3**](#figure_spectrum).

<div id="figure_spectrum" class="figure-combo">

  <img src="res/software/spektrum.svg"/>

  <div><b>Figure 7.3:</b> The frequency spectrum of signal in <a href="#/chapters/7_computer-operations#figure_sampled_a"><b>fig. 7.2</b>a</a></div>

</div>

For explicit signal reconstruction the condition *f*<sub>s</sub>&thinsp;&gt;&thinsp;2*f*<sub>max</sub> must be
satisfied, see [**fig. 7.4**](#figure_sampleok). If a sampling frequency is
lower then 2*f*<sub>max</sub> then the higher frequency components are lost. This
error is called *aliasing*.

<div id="figure_sampleok" class="figure-combo">

  <img src="res/software/signal-VZ.svg"/>

  <div><b>Figure 7.4:</b> The signal sampled with a frequency higher than 2<i>f</i><sub>max</sub></div>

</div>

<div class="figure-combo">

  <img src="res/software/signal-VZbad.svg"/>

  <div><b>Figure 7.5:</b> The sample rate does not meet the Shannon's theorem.</div>

</div>

For the sampling of common narrow band signals like SSTV, RTTY, PSK31 or WEFAX,
which are transferred via SSB channel with a bandwidth of about 2500&ndash;3000&thinsp;Hz;
a sample rate of 11025&thinsp;Hz is enough.
...
signal amplitude scan in a defined range, for 8 bits it is 2<sup>8</sup>&thinsp;=&thinsp;256 values and
for 16 it is 65536.

Constrained resolution of A/D converter causes an *quantization* error. E.g.
for 8-bit converter processing voltage range 0&ndash;5.0&thinsp;V it's the error
5.0&thinsp;/&thinsp;(2<sup>8</sup>&thinsp;-&thinsp;1)&thinsp;≐&thinsp;0.02&thinsp;V. The 8-bit converter cannon distinguish
voltage levels lower then 0.02&thinsp;V. So for input voltage 3.111&thinsp;V it could
find corresponding numeric value `10011110`<sub>2</sub>&thinsp;≈&thinsp;3.098&thinsp;V
or `10011111`<sub>2</sub>&thinsp;≈&thinsp;3.118&thinsp;V because less significant bit is
...
the 16-bit resolution is acceptable.

Modern sound cards could be equipped with digital signal processor (DSP),
which extend card functions e.g. for filtering or data compression during
recording so it can lighten load of main computer CPU. E.g. *Sound Blaster Live!* contains programmable DSP labeled *EMU10K1*.
...
### Interface between TRX and PC

The basic interface is made of shielded cables and 3.5mm jack plugs. A
reception cable connects sound card input *Line In* and TRX headphones
output or output for external speaker. For use of sound card microphone input
can be used TRX output often labeled as `AF OUT` with impedance about
10&thinsp;kΩ which gives max. output voltage 100&thinsp;mV. This output could be also
...
<div class="figure-combo">
  ...
  <img src="res/software/sound_card.svg" style="max-height: 18rem;"/>

  <div><b>Figure 7.6:</b> Basic interface between transceiver and sound card.</div>

</div>

For the transmission it is possible to use *Line Out* with impedance
about 600&thinsp;Ω. The *Line Out* can be connected to microphone
input of TRX or a rear panel connector like `PATCH IN`.
...
<div class="figure-combo">
  ...
  <img src="res/software/hlas.png"/>

  <div><b>Figure 7.7:</b> Software volume control.</div>
...
A program *QuickMix* is can easily store your settings, because
...
- Manual PTT switching. This handy method is not very elegant, but for the
  first experiments can be used.

- Automatic switching can provide TRX with a VOX feature, when the
...
- Automatic PTT switch can control a computer. All SSTV programs support
  ...
  [**fig. 7.8**](#figure_trafo) for details. The control signal is connected to `RTS`
  pin (7 at Cannon DB9 connector, 4 at DB25) or `DTR` (4 at DB9, 20 at DB25), selected pin can
  ...
  100&thinsp;nF and PTT signal is connected by resistor which resistance can by find in
  TRX instruction or you can connect trimming resistor about 15&thinsp;kΩ and
  test the max. value when TRX switching.

- Some transceivers support control over serial port. This CAT (Computer
...
`R2` to lower value when the opto-isolator is not switched properly when
serial port signal is on.

<div id="figure_trafo" class="figure-combo">

  <img src="res/software/sound_if.svg"/>

  <div><b>Figure 7.8:</b> The galvanic separation of transceiver and sound card.</div>

</div>

## Timing oscillator configuration

There is description of synchronous (free-run) SSTV system in
[**Horizontal synchronization**](chapters/3_modern-sstv-features#horizontal-synchronization).
Horizontal synchronization pulses (syncs) are detected
...
If the timing slightly differs then images are distorted &ndash; inaccurate timing
causes image *slant*. You can see image slant for 0.01% timing difference
in [**fig. 7.9**](#figure_slant). If a transmitting station has higher timing (and reception
lower) the image slants to the right [**fig. 7.9**a](#figure_slant_a) in opposite situation to the left
[**fig. 7.9**b](#figure_slant_b).

<div id="figure_slant" class="figure-combo">

  <div id="figure_slant_a" class="figure-item">
    <div class="figure-wrapper">
      <img src="res/software/slant1.png" />
    </div>
    <div>a) Opposite station has higher timing</div>
  </div>

  <div id="figure_slant_b" class="figure-item">
    <div class="figure-wrapper">
      <img src="res/software/slant2.png" />
    </div>
    <div>b) Opposite staion has lower timing</div>
  </div>

  <div><b>Figure 7.9:</b> Image slant distortion when inaccurate timing is used for free-run modes.</div>

</div>

However timing derived from sample rate is not used to be exactly
11025.00&thinsp;Hz, but often can differ up to few tenths of percent for each peace
...
image set the timing &ndash; program will automatically compute timing deviation.
...
The usable transmitter is a Moscow station `RWM` operating on frequencies
4996.0, 9996.0, 14996.0&thinsp;kHz with 8&thinsp;kilowatts power. So it can be nicely
...
then the timing signal will continue. Pulses in intervals 1/60 and 1&thinsp;Hz goes
between 10. and 20. minute and 10&thinsp;Hz pulses goes between 20. and 30. minute.
This is repeated every 30 minutes.

<div class="figure-combo">

  <img src="res/software/rmw_nastaveni.png"/>

  <div><b>Figure 7.10:</b> The configuration of accurate timing with <code>RMW</code> reception in MMSSTV.</div>

</div>

The reception of `WWV` station is the next possibility. This station
broadcast timing pulses and announcement on frequencies 2500.0, 5000.0,
10000.0, 15000.0, 20000.0&thinsp;kHz and uses double sideband (DSB) modulation.
You can receive it with AM mode selected. The `WWV` operates from the North
America, Fort Collins in Colorado. The used power ranges from 2.5 to 10&thinsp;kW.
...
of actual frequency, e.g. f&thinsp;=&thinsp;11024.45&thinsp;Hz or like deviance from *f*<sub>s</sub> the
Δ<sub><i>f</i></sub>&thinsp;=&thinsp;-0.55&thinsp;Hz. Some program this measure in *parts per million (ppm)* unit. The ppm deviance can be computed:

$$\Delta = \frac{\Delta_f}{f_\mathrm{s}}\cdot10^6.$$

For the *f*&thinsp;=&thinsp;11024.45&thinsp;Hz the deviance in ppm is:

$$\Delta =
 \frac{\Delta_f}{f_\mathrm{s}}\cdot10^6=\frac{-0.55}{11025.00}\cdot10^6\,\mathrm{ppm}\doteq-50\,\mathrm{ppm}.$$

### Transmit timing offset

There is necessity to configure transmit timing *TX offset* after the
...
feedback it is practicable to check the TX offset &ndash; deviance between reception
and transmission sample rate. The feedback can be internal or external. *Extenal feedback* needs to connect Line Out and Line In with cable and it
requires a sound card with full-duplex mode enabled. By this way you can set *TX offset* precisely on your own.

The *internal feedback* doing almost the same, but no external cables
...
different sample rates for each channel. This is for example *SB Live! Value*. I have noticed that my TX offset randomly changes! I have this
experiences with SB Live! Value and I found that another radio amateurs has
same. You can try to set other sample rate than 11025.0&thinsp;Hz in this case, if
this doesn't load your computer too much. For example try 48000.0&thinsp;Hz, this
value is fixed sample rate (see your card user's guide) and best results you
can achieve with using of this value or its half or quarter &ndash; 24000.0&thinsp;kHz or
12000.0&thinsp;kHz. When you change this value you need to recalibrate your accurate
timing again.
...
Every SSTV program is equipped with precise tuning indicators &ndash; spectroscopes,
see [**fig. 7.11**](#figure_tuning). The spectroscope shows frequency band from 1000&thinsp;Hz
to 2500&thinsp;Hz with marks for critical frequncies &ndash; 1200&thinsp;Hz for syncs, 1500&thinsp;Hz
and 2300&thinsp;Hz for the video signal.
...
<div id="figure_tuning" class="figure-combo" style="width: 100%;">

  <div class="figure-item" style="width: 12rem;">
    <div class="figure-wrapper" style="height: 12rem;">
      <img src="res/software/chromapix.png" />
    </div>
    <div>Chroma Pix</div>
  </div>
  ...
  <div class="figure-item" style="width: 12rem;">
    <div class="figure-wrapper" style="height: 12rem;">
      <img src="res/software/mmsstv.png" />
    </div>
    <div>MMSSTV</div>
  </div>
  ...
  <div class="figure-item" style="width: 12rem;">
    <div class="figure-wrapper" style="height: 12rem;">
      <img src="res/software/jvfax1.png" />
    </div>
    <div>JVComm32</div>
  </div>

  <div class="figure-item" style="width: 12rem;">
    <div class="figure-wrapper" style="height: 6rem;">
      <img src="res/software/mscan.png" />
    </div>
    <div>MSCAN</div>
  </div>
  ...
  <div class="figure-item" style="width: 12rem;">
    <div class="figure-wrapper" style="height: 6rem;">
      <img src="res/software/qsstv.png" />
    </div>
    <div>QSSTV</div>
  </div>
  ...
  <div class="figure-item" style="width: 12rem;">
    <div class="figure-wrapper" style="height: 6rem;">
      <img src="res/software/mixw.png" />
    </div>
    <div>MixW</div>
  </div>

  <div><b>Figure 7.11:</b> Spectroscopes in common SSTV programs.</div>
...
[](7.6_software-for-windows.md ":include")
